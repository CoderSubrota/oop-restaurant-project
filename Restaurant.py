...
class Admin:
    ...
    @classmethod
    def add_new_admin(self, name, email, address):
        new_admin = Create_Admin_Account(name, email, address)
    ...
    def remove_item(self, item_name):
        for item in self.menu_list:
            if item.item_name.upper() == item_name:
                self.menu_list.remove(item)
                print(f"\n >>> Item {item.item_name} has been removed from the menu!")
                break
            else:
                print(f"\n >>> Item not found with the given item name {item_name}. <<< \n")

    @classmethod
    def add_new_customer(self ,name, email, address):
        new_customer = Add_Customer(name, email, address)
    ...
    def remove_customer(self, customer_name):
        for customer in self.customer_list:
            if customer.name.upper() == customer_name:
                self.customer_list.remove(customer)
                print(f"\n >>> Customer {customer.name} has been removed.")
                break
            else:
             print(f"\n >>> Customer not found with the given name {customer_name}. \n")
    ...
    @classmethod
    def set_item_price(self, item_name, item_price):
        for item in self.menu_list:
        ...
        while True:
            ...
            try:
                ...
                if choice == 1:
                    ...
                    self.remove_customer(customer_name=customer_name)
                elif choice == 6:
                    self.show_menu()
                elif choice == 7:
                    item_name = input(">>> Enter item name: ").upper()
                    item_price = int(input(">>> Enter new item price in BDT: "))
                    self.set_item_price(item_name=item_name, item_price=item_price)
                elif choice == 8:
                    print("\n >>> You are exited from the admin menu.\n")
                    break
                else:
...
class Customer:
    ...
    @classmethod
    def create_new_account(self, name, email, address):
        new_customer = Add_Customer(name=name, email=email, address=address)
...
class Restaurant:
    ...
    def register_or_login_customer():
           ...
           if option.lower()=='l':
            ...
            if (Customer.create_account() ==False):
                return False
            else:
              Customer.customer_menu()
